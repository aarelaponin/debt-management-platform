...
/**
 * MatrixValidator (DAS finalisation P5) — a pure consistency gate for an {@code mmAuthority} action's
 * bands, so a bad authority configuration is caught BEFORE it routes a real decision. Validates, for
 * the bands of one action type:
 * <ul>
 ...
 * </ul>
 * Free of Joget types so it is exhaustively unit-testable; {@code AuthorityMatrixEngine} feeds it the
 * live rows.
 */
