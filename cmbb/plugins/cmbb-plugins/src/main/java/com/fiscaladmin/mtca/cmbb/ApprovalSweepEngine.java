...
/**
 * ApprovalSweepEngine — the time-driven half of the Decision &amp; Approval Service (#6). Form
 * post-processor on {@code cmApprovalSweep} create: runs {@link com.fiscaladmin.mtca.cmbb.service.ApprovalService#sweep}
 * over the Pending requests as of {@code asOf} (blank = now; a bare date = start of that day —
 * lets an operator time-travel the SLA clock). Overdue requests escalate one rank up, then time
 * out (auto-reject) once escalations are exhausted.
 */
public class ApprovalSweepEngine extends DefaultApplicationPlugin {
    ...
    public String getName() {
        return "DMBB Approval Sweep Engine";
    }
    ...
    public String getDescription() {
        return "Decision & Approval Service #6 — SLA sweep: escalate then time out overdue requests.";
    }
    ...
    public Object execute(Map properties) {
        ...
        LocalDateTime asOf = parseAsOf(DeadlineService.prop(run, "asOf"));
        String result = ApprovalEffects.service(dao).sweep(asOf, actor);
