...
/**
 * Config-driven status state machine for DMBB subject/child entities (ADR-003).
 * Native + Joget-only: the lifecycle lives in the {@code mmEntityState} /
 * {@code mmEntityTransition} forms (read via {@link MmConfigService}); the status
 * is written via {@link FormDataDao} (which bypasses the read-only form attribute —
 * engines only); each accepted transition appends a {@code STATUS_CHANGED} row to
 * the {@code cmEvent} hash-chain via {@link CaseEventWriter}. No external
 * status-framework, no new database, no raw SQL, no new process (D-SM-01..05).
 *
 * <p><b>Scope resolution (per-tax-type, D-SM-03):</b> most-specific-first over the
 * supplied chain (e.g. {@code taxType, caseType}); the first scope that has ANY
 * transition rows wins WHOLLY ("wholesale shadowing") — its lifecycle fully
 * replaces the less-specific ones. {@code DEFAULT} is always the final fallback.
 *
 * <p>Dormant in the foundation increment: built + unit-tested, not yet called by
 * the engines (they are migrated entity-by-entity in later passes, ADR §5).
 */
