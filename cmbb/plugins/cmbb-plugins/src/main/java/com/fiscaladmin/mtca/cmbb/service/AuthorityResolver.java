...
/**
 * AuthorityResolver (DAS finalisation P3) — resolve an approver's rank level from their DIRECTORY
 * identity, not a self-declared field. API-only by design:
 * <ul>
 ...
 *       {@code mmRoleLevel} config carrier), overlaid on a sensible default map so the gate works
 *       out-of-the-box and is still configurable.</li>
 * </ul>
...
public final class AuthorityResolver {
    ...
    /** Default role-group &rarr; rank-level map (overridden/extended by mmRoleLevel rows). */
    static final Map<String, String> DEFAULT_MAP = new HashMap<String, String>();
    static {
        DEFAULT_MAP.put("dm_officer", "OFFICER");
        DEFAULT_MAP.put("dm_supervisor", "SUPERVISOR");
        DEFAULT_MAP.put("dm_manager", "MANAGER");
        DEFAULT_MAP.put("dm_policy_admin", "DIRECTOR");
    }
    ...
    Map<String, String> roleLevelMap() {
        ...
        try {
            ...
            if (rows != null) {
                for (FormRow r : rows) {
                    String g = DeadlineService.prop(r, "roleGroup");
                    String l = DeadlineService.prop(r, "level");
        ...
        return new GroupSource() {
            ...
            public List<String> groupsOf(String username) {
                ...
                try {
                    ...
                    // directory unavailable / pseudo-user (e.g. roleAnonymous) → no groups.
                }
                return ids;
            }
        };
