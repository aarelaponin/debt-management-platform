...
/**
 * ApprovalInbox (DAS finalisation P3b) — the pure eligibility rule behind the per-user
 * "approvals mine to decide" inbox. Given the Pending requests and the current user's
 * directory-resolved rank level, it returns the subset that THIS user may actually decide:
 * <ul>
 ...
 * </ul>
 * Kept free of any Joget/runtime types so it is exhaustively unit-testable; the
 * {@code ApprovalInboxBinder} supplies the live rows + resolved level.
 */
