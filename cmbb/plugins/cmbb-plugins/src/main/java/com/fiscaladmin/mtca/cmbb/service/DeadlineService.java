...
/**
 * SLA clock engine core (CMBB-F05, WF-FR-010..012, CAD case_deadline state
 * machine). Clocks are cmDeadline rows created from mmSla configuration;
 * SWEEP recomputes statuses, pauses/resumes on hold, and escalates —
 * idempotently via escalationLevel (breach escalation fired exactly once).
 */
public class DeadlineService {

    public static final String F_DEADLINE = "cmDeadline";
    ...
    public DeadlineService(FormDataDao dao, MmConfigService mm, CaseEventWriter events) {
        this.dao = dao;
        this.mm = mm;
        this.events = events;
    }

    /** START: one clock per mmSla row of the case type. Returns clocks created. */
    public int start(String caseId, String actor, LocalDateTime now) {
        ...
        FormRowSet slas = mm.slaRows(type);
        int created = 0;
        if (slas != null) {
            for (FormRow sla : slas) {
                ...
                LocalDateTime due = addDays(now, days, mm.calendarRow(prop(sla, "calendar")));
    ...
    public String sweep(String actor, LocalDateTime asOf) {
        FormRowSet clocks = dao.find(F_DEADLINE, F_DEADLINE,
        ...
        if (clocks != null) {
            for (FormRow d : clocks) {
                ...
                FormRow sla = mm.slaRow(type, prop(d, "clockCode"));
                boolean pauseOnHold = sla == null
                        || !"".equals(prop(sla, "pauseOnHold")); // default true
                boolean onHold = "OnHold".equals(mm.envelopeOf(type, state));
                ...
                long maxLevels = AllocationService.jsonInt(chain, "maxLevels", 3);
    ...
    private void escalate(FormRow c, FormRow d, FormRow sla, int level,
        ...
        String chain = sla == null ? "" : prop(sla, "escalationChain");
        String extra = "\"caseRef\":\"" + CaseEventWriter.esc(prop(c, "caseRef"))
                + "\",\"taxpayer\":\"" + CaseEventWriter.esc(prop(c, "taxpayerName"))
                + "\",\"amount\":\"" + CaseEventWriter.esc(prop(c, "amountAtStake"))
                + "\",\"slaStatus\":\"RED\",\"daysOverdue\":" + daysOverdue
                + ",\"clockCode\":\"" + CaseEventWriter.esc(prop(d, "clockCode"))
                + "\",\"level\":" + level;
        events.append(caseId, eventType, actor, "", "",
                "SLA escalation level " + level, extra);
        List<String> notify = jsonList(chain, "notify");
        if (notify.isEmpty()) {
            notify = List.of("assignee", "supervisor"); // WF-FR-012 default
        }
        for (String who : notify) {
            String recipient = "assignee".equals(who) ? prop(c, "assignee")
                    : "supervisor".equals(who) ? "dm_supervisor" : who;
    ...
    private List<FormRow> openClocks(String caseId) {
        FormRowSet rows = dao.find(F_DEADLINE, F_DEADLINE,
        ...
        return rows == null ? new java.util.ArrayList<FormRow>() : rows;
    }
    ...
    public static LocalDateTime parse(String s, LocalDateTime dflt) {
        try {
    ...
    /** JSON string-array extractor — NO default (unlike AllocationService's). */
    static List<String> jsonList(String json, String key) {
        java.util.regex.Matcher m = java.util.regex.Pattern
                .compile("\"" + key + "\"\\s*:\\s*\\[([^\\]]*)\\]").matcher(nz(json));
