...
def element(f):
    ...
    if t == "radio":
        return {"className": "org.joget.apps.form.lib.Radio", "properties": {
            "id": fid, "label": label, "value": "", "fullWidth": "", "readonly": "",
    ...
    if t == "date":
        return {"className": "org.joget.apps.form.lib.DatePicker", "properties": {
            ...
            "datePickerType": "", "currentDateAs": "", "yearRange": "c-5:c+10",
    ...
    if t == "subform":
        ...
        # debt case's dmDebt row shares the cmCase id — no foreign key needed.
    ...
    if t == "grid":
        # 1:many editable child grid (e.g. dmLine on a debt case). Child rows are
...
def post_processor(spec):
    """Optional form post-processor (tool run after save) — CMBB-F03+."""
