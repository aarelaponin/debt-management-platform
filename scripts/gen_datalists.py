...
custom datalist specs (binder: jdbc|form). UX layer (DMBB-UX-QA, 2026-06-13):
...
UV = "#request.contextPath#/web/userview/dmbb/dmbbConsole/_"  # detail-list / record drill base

# UX-QA2/QA3: form-field labels carry authoring annotations that help a data-entry user but are pure
# noise as a grid column header — provenance notes ("(set by engine)"), FK/source-table refs
# ("(mdInstrument)", "(dmCharge)"), explanatory glosses ("(debt being enforced)", "(copied from case)")
...
# debt-category code range. (QA2 only stripped a fixed leak-keyword set, which left the glosses above.)
...
def col(idx, name, label, fmt=None, action=None, render_html=False):
    """Full canonical column shape — ALWAYS sortable (UX-QA); label cleaned of provenance leaks (UX-QA2)."""
...
def main():
    ...
    for fn in sorted(os.listdir(spec_dir)):
        ...
        if "datalist" in spec:  # custom datalist spec (CMBB-F03+)
            d = spec["datalist"]
            cols = []
        ...
        # UX-QA: form-row binder applies datalist filters natively, so auto-add a "contains" filter on
        # the common operational columns when the form has them (TIN / status / category / instrument / type).
        auto_filter = {"tin": "TIN", "status": "Status", "debtCategory": "Category",
                       "instrument": "Instrument", "woType": "Type", "agentId": "Agent"}
        # UX-QA3: optional `listColumns` (ordered field ids) curates a wide form's companion list to a
